...
ggplot(data) +
        ...
        labs(title="Tweets do not come in strong clusters") +
        xlab("PC1") +
        ylab("PC2") +
...
ggplot(data) +
        geom_point(aes(x = x, y = y), alpha = 0.4, color = "blue") +
        labs(title="Tweets are mostly distinct, with occasional clusters") +
        xlab("PC1") +
        ylab("PC2") +
        theme_few(base_family = "GillSans")
...
ggplot(data) +
        ...
        labs(title="Tweet feature space, after vectorization") +
        xlab("PC1") +
        ylab("PC2") +
        theme_few(base_family = "GillSans")
...
$C = m + (1-m)\left(\frac{t_{min}}{t_{prev}}\right)$,

where $C$ is the conformity score percentile threshold function, $t_{min}$ is the minimum amount of time (in hours) that must pass before a new notification is sent, and $t_{prev}$ is the amount of time that has passed since the previous notification was sent. Below you can see two versions of the threshold function. The blue curve has $t_{min} = 1$, and the green curve has $t_{min} = 5$. You can see that if the $t_{min}$ were set to $5$, we would be forcing the system to wait a full five hours before any tweet could have a chance of being sent as a notification. After that, the minimum percentile decreases steadily as time passes.
